...
** 
** Implementation file for lexer generation functionality.
** Extracted and adapted from lemonex parser generator.
**
** This library provides lexer generation functionality that can be used
** independently to generate lexical analyzers from regular expressions.
**
...
/* Character class list containing Unicode ranges for letter, digit, word, and space classes */
...
/* =============================================================================
** UTF-8 UTILITY FUNCTIONS (lines 4020-4139 from lemon.c)
** ===========================================================================*/
...
struct LxState *LxState_create(struct LexerContext *ctx){
  ...
  new_state->state_id = ++(ctx->next_lxstate_id);
  new_state->is_leaf = 0;
  ...
  if(ctx->last_state == 0){
    ...
    ctx->last_state->next_state = new_state;
  }
...
struct LxMode *LxMode_new(struct LexerContext *ctx, const char *mode_name, int is_nesting){
  struct LxMode *new_mode = LxMode_find(ctx, mode_name);
  if(new_mode != 0){
    return new_mode;
  }
  ...
  if(new_mode == 0){
    fprintf(stderr, "Out of memory allocating new lexer mode.\n");
    exit(1);
  }
  new_mode->name = mode_name;
  new_mode->init_state = LxState_create(ctx);
  new_mode->is_nesting = is_nesting;
  new_mode->next_mode = 0;

  if(ctx->first_mode == 0){
    ctx->first_mode = new_mode;
    ctx->last_mode = new_mode;
  }else{
    ctx->last_mode->next_mode = new_mode;
    ctx->last_mode = new_mode;
  }
  return new_mode;
}

static struct LxAstNode *LxNode_create(int node_type, struct LxAstNode *lhs, struct LxAstNode *rhs, int ch0, int ch1){
...
/* =============================================================================
** DEBUG AND COMPARISON FUNCTIONS (lines 4242-4445 from lemon.c)
** ===========================================================================*/
...
/* =============================================================================
** RANGE ANALYSIS FUNCTIONS (lines 4447-4518 from lemon.c)
** ===========================================================================*/

static int LxRange_contains(struct LexerContext *ctx, int rch0, int rch1, int cch0, int cch1, int is_cleaf){
  /* cch is a range, skip */
  if(cch1 != 0){
    return 0;
  }
  ...
  if(rch0 == 0){
    ...
    int i = 0;
    while(ctx->lxclasslist[i+1] != 0) {
      if((ctx->lxclasslist[i] == 0) && (ctx->lxclasslist[i+1] == cls)) {
        break;
      }
      i += 2;
    }
    if(ctx->lxclasslist[i+1] == 0) {
      printf("INTERNAL-ERROR:unhandled classtype:%d(%c)\n", cls, cls);
      exit(1);
    }

    i+=2;
    while(ctx->lxclasslist[i] != 0) {
      if((ctx->lxclasslist[i] <= cch0) && (cch0 <= ctx->lxclasslist[i+1])) {
        return 1;
      }
      i += 2;
    }
...
/* =============================================================================
** STATE DROP/GOTO FUNCTIONS (lines 4543-4551 from lemon.c)
** ===========================================================================*/

void LxState_set_drop(struct LxState *curr_state, struct LxState *drop_state){
  assert(curr_state->drop_state == 0);
  curr_state->drop_state = drop_state;
}

void LxState_set_goto(struct LxState *curr_state, struct LxState *goto_state){
...
/* =============================================================================
** AST PROCESSING FUNCTIONS (lines 4553-4762 from lemon.c)
** ===========================================================================*/

/* Context structure for processing - mimics pstate fields */
...
static struct LxState *LxNode_process_internal(struct ProcessContext *pctx, int lvl, char pos, struct LxAstNode *curr_node, struct LxState *curr_state, int is_chk);

static struct LxTransition *LxTransition_insert_node(struct ProcessContext *pctx, struct LxAstNode *curr_node, int tx_type, struct LxState *curr_state, struct LxState *new_state){
  struct LxTransition *new_transition = LxTransition_create(curr_node->ch0, curr_node->ch1, tx_type, new_state);
  new_transition->is_capturing = pctx->is_capturing;
  LxTransition_insert_at(curr_state, new_transition);
  return new_transition;
}

static struct LxState *LxNode_process_charnode(struct ProcessContext *pctx, struct LxAstNode *curr_node, struct LxState *curr_state, int is_chk){
  ...
  if(tx1 != 0){
    if(tx1->next_state->is_loop_return == 1){
      assert(tx1->is_capturing == pctx->is_capturing);
  ...
  struct LxState *new_state = LxState_create(pctx->ctx);
  LxTransition_insert_node(pctx, curr_node, REGULAR_TX, curr_state, new_state);
  ...
  if(curr_state->star_state != 0){
    ...
    if(tx2 != 0){
      ...
      LxTransition_insert_node(pctx, curr_node, REGULAR_TX, curr_state->star_state, new_state);
    }
  }
...
static struct LxState *LxNode_process_classnode(struct ProcessContext *pctx, struct LxAstNode *curr_node, struct LxState *curr_state, int tx_type, int is_chk){
  ...
  new_state = LxState_create(pctx->ctx);
  for(struct LxAstNode* node = curr_node->rhs; node != 0; node = node->rhs){
    struct LxTransition *dtx = LxTransition_insert_node(pctx, node, tx_type, curr_state, new_state);
    dtx->tx_type = tx_type;
  }
...
static struct LxState *LxNode_process_loopnode(struct ProcessContext *pctx, int lvl, struct LxAstNode *curr_node, struct LxState *curr_state, int is_chk, int is_star){
  ...
  for(struct LxTransition* tx = curr_state->head_transition; tx != 0; tx = tx->next_transition){
    ...
    lhs = LxNode_process_internal(pctx, lvl+1, 'L', curr_node->lhs, tx->next_state, 1);
    if(lhs != 0){
  ...
  lhs = LxNode_process_internal(pctx, lvl+1, 'L', curr_node->lhs, new_state, is_chk);
  assert(lhs != 0);
  ...
  for(struct LxTransition* tx = new_state->head_transition; tx != 0; tx = tx->next_transition){
    ...
    if(tx1 != 0){
      ...
      struct LxState *dup_state = LxState_create(pctx->ctx);
      dup_state->is_loop_enter = 1;
      LxState_set_goto(dup_state, tx->next_state);
      struct LxTransition *dup_transition = LxTransition_create(tx->ch0, tx->ch1, tx->tx_type, dup_state);
      dup_transition->is_capturing = tx->is_capturing;
      ...
      for(struct LxTransition* tx2 = new_state->head_transition; tx2 != 0; tx2 = tx2->next_transition){
        ...
        if(tx->next_state == tx2->next_state){
          dup_transition = LxTransition_create(tx2->ch0, tx2->ch1, tx2->tx_type, dup_state);
          dup_transition->is_capturing = tx2->is_capturing;
          LxTransition_insert_at(curr_state, dup_transition);
          tx2->is_loop_set = 1;
        }
      }
  ...
  struct LxTransition *loop_transition = LxTransition_create(0, 0, LOOP_TX, new_state);
...
static struct LxState *LxNode_process_internal(struct ProcessContext *pctx, int lvl, char pos, struct LxAstNode *curr_node, struct LxState *curr_state, int is_chk){
  ...
  switch(curr_node->node_type){
    case CHAR_NODE:
      return LxNode_process_charnode(pctx, curr_node, curr_state, is_chk);
    case CLASS_NODE:
      return LxNode_process_classnode(pctx, curr_node, curr_state, REGULAR_TX, is_chk);
    case NCLASS_NODE:
      return LxNode_process_classnode(pctx, curr_node, curr_state, NOT_TX, is_chk);
    case STAR_NODE:
      return LxNode_process_loopnode(pctx, lvl, curr_node, curr_state, is_chk, 1);
    case PLUS_NODE:
      return LxNode_process_loopnode(pctx, lvl, curr_node, curr_state, is_chk, 0);
    case OPT_NODE:
      assert(curr_node->lhs != 0);
      assert(curr_node->rhs == 0);
      lhs = LxNode_process_internal(pctx, lvl+1, 'L', curr_node->lhs, curr_state, is_chk);
      assert(lhs != 0);
    ...
    case OR_NODE:
      ...
      assert(curr_node->rhs != 0);
      lhs = LxNode_process_internal(pctx, lvl+1, 'L', curr_node->lhs, curr_state, is_chk);
      rhs = LxNode_process_internal(pctx, lvl+1, 'R', curr_node->rhs, curr_state, is_chk);
      assert(lhs != 0);
      assert(rhs != 0);
    ...
    case AND_NODE:
      ...
      assert(curr_node->rhs != 0);
      lhs = LxNode_process_internal(pctx, lvl+1, 'L', curr_node->lhs, curr_state, is_chk);
      rhs = LxNode_process_internal(pctx, lvl+1, 'R', curr_node->rhs, lhs, is_chk);
      assert(lhs != 0);
      assert(rhs != 0);
      return rhs;
  }
...
struct LxState *LxNode_process(
  ...
  pctx.ctx = ctx;
  pctx.is_capturing = 1;  /* Default to capturing */
  pctx.nstarcount = 0;
  (void)final_state;  /* Unused parameter */
  return LxNode_process_internal(&pctx, 0, '*', node, start_state, 0);
}


/* =============================================================================
** REGEX PARSING FUNCTIONS (lines 4764-4915 from lemon.c)
** ===========================================================================*/

static struct LxAstNode *lx_expr_internal(struct LxScanner *s);

static struct LxAstNode *lx_primary_expr(struct LxScanner *s, int chk_range){
...
struct LxAstNode *lx_or_expr(struct LxScanner *s){
  ...
  while(lx_lookahead(s) == '|') {
    lx_advance(s);
    struct LxAstNode *rhs = lx_expr_internal(s);
...
static struct LxAstNode *lx_and_expr(struct LxScanner *s){
  ...
  struct LxAstNode *rhs = 0;
  while((rhs = lx_expr_internal(s)) != 0){
...
struct LxAstNode *lx_expr(struct LexerContext *ctx, struct LxScanner *s){
  ...
  return lx_expr_internal(s);
}


/* =============================================================================
** STATE POSTPROCESSING FUNCTIONS (lines 4917-5081 from lemon.c)
** ===========================================================================*/
...
static void LxState_setdrop_at(struct LexerContext *ctx, struct LxState *next_state, struct LxState *drop_state){
...
static void LxState_postprocess_setdrop(struct LexerContext *ctx, int lvl, struct LxState *curr_state){
  ...
  for(struct LxTransition* ctx_tx = curr_state->head_transition; ctx_tx != 0; ctx_tx = ctx_tx->next_transition){
    int found = 0;
    for(struct LxTransition* rtx = curr_state->head_transition; rtx != 0; rtx = rtx->next_transition){
      if(LxRange_contains(ctx, rtx->ch0, rtx->ch1, ctx_tx->ch0, ctx_tx->ch1, ctx_tx->next_state->is_leaf) == 1){
        LxState_setdrop_at(ctx, ctx_tx->next_state, rtx->next_state);
        found = 1;
      }
    }
  ...
  for(struct LxTransition* ctx_tx = curr_state->head_transition; ctx_tx != 0; ctx_tx = ctx_tx->next_transition){
    LxState_postprocess_setdrop(ctx, lvl+1, ctx_tx->next_state);
  }
}

static void LxState_postprocess_duptransition(struct LexerContext *ctx, struct LxState *curr_state, struct LxState *from_state, struct LxState *entry_state){
  (void)ctx;  /* Unused parameter */
  for(struct LxTransition* tx = from_state->head_transition; tx != 0; tx = tx->next_transition){
    ...
    struct LxTransition *dup_transition = LxTransition_create(tx->ch0, tx->ch1, INIT_TX, tx->next_state);
    dup_transition->is_capturing = tx->is_capturing;
    LxTransition_insert_at(curr_state, dup_transition);
  }
}

static void LxState_postprocess_setinit(struct LexerContext *ctx, int lvl, struct LxState *curr_state, struct LxState *entry_state){
  if(curr_state->is_processed == 1){
    return;
  }
  curr_state->is_processed = 1;
  ...
  for(struct LxTransition* ctx_tx = curr_state->head_transition; ctx_tx != 0; ctx_tx = ctx_tx->next_transition){
    ...
    if(ctx_tx->tx_type == NOT_TX){
      if((curr_state->not_transition != 0) && (curr_state->not_transition->next_state != ctx_tx->next_state)){
        printf("ERROR: Multiple overlapping not-transitions at the same state\n");
        exit(1);
      }
      if(curr_state->not_transition == 0){
        curr_state->not_transition = ctx_tx;
      }
    }

    /* and dot_transition as well */
    if((ctx_tx->ch0 == 0) && (ctx_tx->ch1 == LXCLASS_DOT)){
      if(curr_state->dot_transition != 0){
        printf("ERROR: Multiple dot-transitions at the same state\n");
        exit(1);
      }
      curr_state->dot_transition = ctx_tx;
    }
  }

  if(curr_state->is_leaf == 1){
    if(curr_state->drop_state != 0){
      LxState_postprocess_duptransition(ctx, curr_state, curr_state->drop_state, entry_state);
    }

    if(curr_state->dot_transition != 0){
      assert(curr_state->init_mode);
      assert(curr_state->init_mode->init_state);
      LxState_postprocess_duptransition(ctx, curr_state, curr_state->init_mode->init_state, entry_state);
    }
  }else{
    for(struct LxTransition* ctx_tx = curr_state->head_transition; ctx_tx != 0; ctx_tx = ctx_tx->next_transition){
      if((ctx_tx->ch1 != 0) || (ctx_tx->ch0 != 0)){
        continue;
      }
      if(ctx_tx->next_state->is_leaf == 0){
        if(ctx_tx->next_state->goto_state == 0){
          LxState_set_goto(ctx_tx->next_state, curr_state->goto_state);
        }
      }
    }
  }
  ...
  for(struct LxTransition* ctx_tx = curr_state->head_transition; ctx_tx != 0; ctx_tx = ctx_tx->next_transition){
    ...
    if(es == 0){
      es = ctx_tx->next_state;
    }
    LxState_postprocess_setinit(ctx, lvl+1, ctx_tx->next_state, es);
  }
}

void LxState_postprocess(struct LexerContext *ctx, struct LxState *state){
  (void)state;  /* Unused parameter - process all states */
  /* Process all modes starting from their init states */
  for(struct LxMode *curr_mode = ctx->first_mode; curr_mode != 0; curr_mode = curr_mode->next_mode){
    LxState_postprocess_setdrop(ctx, 0, curr_mode->init_state);
  }
  for(struct LxState* curr_state = ctx->first_state; curr_state != 0; curr_state = curr_state->next_state){
    curr_state->is_processed = 0;
  }
  for(struct LxMode *curr_mode = ctx->first_mode; curr_mode != 0; curr_mode = curr_mode->next_mode){
    LxState_postprocess_setinit(ctx, 0, curr_mode->init_state, 0);
  }
}


/* =============================================================================
** CODE GENERATION FUNCTIONS (lines 7253-7518 from lemon.c)
** ===========================================================================*/

void Lexer_generate_class_list(struct LexerContext *ctx, FILE *fp, int *lineno){
  int i = 0;
  while(ctx->lxclasslist[i+1] != 0) {
    if(ctx->lxclasslist[i] == 0) {
      if(i > 0){
        fprintf(fp, "  0,0\n");(*lineno)++;
        fprintf(fp, "};\n");(*lineno)++;
      }
      fprintf(fp, "static int lxcls_%c[] = {\n", ctx->lxclasslist[i+1]);(*lineno)++;
    }else{
      fprintf(fp, "  0x%-6X,0x%-6X,", ctx->lxclasslist[i], ctx->lxclasslist[i+1]);(*lineno)++;
      if((i%8) == 0){
        fprintf(fp, "\n");
      }
    }
...
static void lx_generate_send(FILE* out, struct LxState *curr_state, const char *token_prefix){
  ...
  if(curr_state->matchedsym != 0){
    const char *prefix = token_prefix ? token_prefix : "";
    /* struct symbol has 'name' as first field, so we can cast safely */
    const char **sym_ptr = (const char **)curr_state->matchedsym;
...
static void lx_generate_conditions(FILE* out, int *lineno, struct LxState *curr_state, struct LxTransition *tx){
...
static void lx_generate_leaf(struct LexerContext *ctx, FILE* out, int *lineno, struct LxState *curr_state, int tx_type, const char *token_prefix){
  (void)ctx;  /* Unused parameter */
  if(tx_type == NOT_TX){
    if(curr_state->dot_transition == 0){
      lx_generate_send(out, curr_state, token_prefix);
    }
    return;
  }
...
static void lx_generate_jump(struct LexerContext *ctx, FILE* out, int *lineno, struct LxState *curr_state, int tx_type){
  ...
  if(curr_state->is_leaf == 1) {
    ...
    if(curr_state->next_mode != 0){
      if(strcmp(curr_state->next_mode, "<") != 0){
        struct LxMode* next_mode = LxMode_find(ctx, curr_state->next_mode);
        if(next_mode == 0) {
...
#endif
      fprintf(out, "LX_SENDERR(ERROR_TOKEN);return -1;");
    }
  }
  if(tx_type == REGULAR_TX){
    fprintf(out, "\n");(*lineno)++;
...
void lexgen_generate_code(
  ...
  const char *token_prefix = "";  /* Can be customized */
  
  fprintf(output, "  LX_ADVANCE(lxpLexer->lxstate);\n");(*lineno)++;
  fprintf(output, "L0:\n");(*lineno)++;
  if (ctx->debug_level >= 1) {
    fprintf(output, "  printf(\"L0:%%d\\n\", lxpLexer->lxstate);\n");(*lineno)++;
  }
  fprintf(output, "  switch(lxpLexer->lxstate) {\n");(*lineno)++;
  for(struct LxState* curr_state = ctx->first_state; curr_state != 0; curr_state = curr_state->next_state){
    fprintf(output, "    case %d:goto S%d;\n",
  ...
  for(struct LxState* curr_state = ctx->first_state; curr_state != 0; curr_state = curr_state->next_state){
    ...
    if (ctx->debug_level >= 1) {
      fprintf(output, "  printf(\"S%d(%%d,%%d):0x%%X(%%c)\\n\", lxpLexer->lxrow, lxpLexer->lxcol, ch, ((ch>= 32) && (ch <= 127))?ch:' ');\n"
        ,curr_state->state_id
      );(*lineno)++;
    }
    if (ctx->debug_level >= 2) {
      LX_PRINT_STATE(output, curr_state);
    }

    for(struct LxTransition* tx = curr_state->head_transition; tx != 0; tx = tx->next_transition){
      LX_PRINT_TRANSITION(output, curr_state, tx);
      if((tx->ch0 == 0) && (tx->ch1 == 0)){
        fprintf(output, "  /*loop --> %d*/\n", tx->next_state->state_id);(*lineno)++;
      }else{
        lx_generate_conditions(output, lineno, curr_state, tx);
        fprintf(output, "{");
        if(tx->tx_type == NOT_TX){
          lx_generate_leaf(ctx, output, lineno, curr_state, 1, token_prefix);
          lx_generate_jump(ctx, output, lineno, curr_state, 1);
        }else{
          if(curr_state->is_leaf == 1){
            if((curr_state->not_transition != 0) && (curr_state->not_transition != tx)){
              if(tx->tx_type == INIT_TX){
                lx_generate_send(output, curr_state, token_prefix);
              }
          ...
          if(tx->is_capturing == 1){
            fprintf(output, "LX_CAPTURE(curr_pos, curr_len);");
          }
          fprintf(output, "LX_ADVANCE(%d);goto S%d; ", tx->next_state->state_id, tx->next_state->state_id);
        }
        fprintf(output, "}");
        fprintf(output, "\n");(*lineno)++;
      }
    ...
    if(curr_state->not_transition != 0){
      lx_generate_leaf(ctx, output, lineno, curr_state, 0, token_prefix);
      fprintf(output, "  ");
      if(curr_state->not_transition->is_capturing == 1){
        fprintf(output, "LX_CAPTURE(curr_pos, curr_len);");
      }
      fprintf(output, "LX_ADVANCE(%d);goto S%d;\n", curr_state->not_transition->next_state->state_id, curr_state->not_transition->next_state->state_id);(*lineno)++;
    }else{
      lx_generate_leaf(ctx, output, lineno, curr_state, 0, token_prefix);
      lx_generate_jump(ctx, output, lineno, curr_state, 0);
    }
  }
}
...
void lexgen_init(struct LexerContext *ctx, const int *classlist){
  ctx->next_lxstate_id = 0;
  ctx->first_mode = 0;
  ctx->last_mode = 0;
  ctx->first_state = 0;
  ctx->last_state = 0;
  ctx->curr_state = 0;
  ctx->lxclasslist = classlist ? classlist : s_lxclasslist;
  ctx->nesting_depth = 0;
  ctx->debug_level = 0;
}
...
struct LxState *lexgen_parse_regex(
...
#if LEMONEX_DBG>=1
  printf("reading regex '%s' as symbol '%s' in mode '%s'\n", regex, (sym)?sym->name:"", mode_name);
#endif

  /* Find or create the mode */
  struct LxMode *mode = LxMode_find(ctx, mode_name);
  if(mode == 0){
    mode = LxMode_new(ctx, mode_name, 0);
  }

  /* Set current state to mode's init state */
  ctx->curr_state = mode->init_state;
  ...
  s.ch = 0;
  s.offset = 0;
  struct LxAstNode *root_node = lx_expr(ctx, &s);
  ...
  /* Process the AST and build state machine */
  struct ProcessContext pctx;
  pctx.ctx = ctx;
  pctx.is_capturing = is_capturing;
  pctx.nstarcount = 0;
  
  ctx->curr_state = LxNode_process_internal(&pctx, 0, '*', root_node, ctx->curr_state, 0);
  
  /* Mark final state */
  ctx->curr_state->is_leaf = 1;
  ctx->curr_state->matchedsym = sym;
